...
-(void)loadLoginView
{
    ...
    [_BtnLoginVistors addTarget:self  action:@selector(eventsQuickLogin:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark --- 设置游客登录按钮事件 ---
-(void)eventsQuickLogin:(id)sender
{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    id accountid=[userDefaults valueForKey:@"AccountID"];
    id localToken=[userDefaults valueForKey:@"Token"];
    NSDictionary* defaults = [userDefaults dictionaryRepresentation];
    NSLog(@"Defaults: %@", defaults);
    if(accountid==nil&&localToken==nil)
    {
        NSLog(@"无值传入");
    }
    else{
//        [TYSDKDataModel sharedInstance].userModel.event=[NSString stringWithFormat:@"%@",localToken];
        [TYSDKDataModel sharedInstance].userModel.event=@"token";
        [TYSDKDataModel sharedInstance].accountModel.accountId=accountid;
        [TYSDKDataModel sharedInstance].userModel.token=localToken;
    }
    [[TYSDKNetW shareInstance]createSession:^(NSDictionary *jsonDic) {
        [TYSDKResponseBase analyTheDic:jsonDic];
        [[loginSuccessViewController alloc]loadSuccessView:^(UIView * _Nonnull displayVc) {
            NSLog(@"laji");
        }];
    }];
}
...
-(void)EventsLogin:(id)sender
{
    ...
    if(![self isValidateEmail:_accountFd.text])
    {
        ...
        if(![self isValidatePassword:_passwordFd.text])
        {
            ...
            [TYSDKDataModel sharedInstance].userModel.event=@"signin";
...
-(void)showSucceessLoginMessage:(NSString *)message
{
    // 1.创建UIAlertController
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示"
                                                                             message:message
...
{
//    [self presentViewController:_loginView animated:NO completion:nil];
}
@end
