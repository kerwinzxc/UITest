...
//  TYSDK
//
//  Created by tianyou_ios on 2019/1/10.
...
@implementation TYforgetPswView
...
-(void)loadForgetPswView
{
    ...
    [_btnZhaoHuiPsw addTarget:self action:@selector(zhaoHui:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)zhaoHui:(id)sender
{
    if(![self isValidateEmail:_emailFd.text])
    {
        [self showMessage:@"请输入有效的邮箱"];
        NSLog(@"请输入有效的邮箱");
    }else{
        [TYSDKDataModel sharedInstance].userModel.email=_emailFd.text;
        [TYSDKDataModel sharedInstance].userModel.event=@"password-reset-request";
